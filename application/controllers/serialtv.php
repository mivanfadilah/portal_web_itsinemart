...
class serialtv extends CI_Controller {
	...
	function index() {
		...
		$limit = 5;
		$config['base_url'] = site_url() . 'serialtv/index/';
		...
		$x['data'] = $this->m_serialtv->serialtv_perpage($offset, $limit);
		// var_dump($this->m_serialtv->serialtv_perpage($offset, $limit)->result_array());
		...
		$this->load->view('serialtv', $x);
	}
	function detail($slugs) {
		$slug = htmlspecialchars($slugs, ENT_QUOTES);
		$query = $this->db->get_where('tb_serialtv', array('slug' => $slug));
		if ($query->num_rows() > 0) {
			$b = $query->row_array();
			$kode = $b['id'];
			$data = $this->m_serialtv->get_serialtvd_by_kode($kode);
			$row = $data->row_array();
			// var_dump($data);
			$x['id'] = $row['id'];
			$x['title'] = $row['judul'];
